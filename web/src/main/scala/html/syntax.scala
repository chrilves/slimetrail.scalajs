...
object syntax:
  ...
  inline final def onInputElement[A](ext: HTMLInputElement => A): Parameter[A] =
  ...
  inline final def oninput[A](reaction: String => A): Parameter[A] =
    onInputElement[A](i => reaction(i.value))

  inline final def oncheck[A](reaction: Boolean => A): Parameter[A] =
